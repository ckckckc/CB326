/*
    ...
    並設計一class為PokemonHealthCenter
    為class Pokemon 的friend class
    使其可以直接存取Pokemon 的私有成員，幫助Pokemon直接回血並印出 xxx has restored health at yyy
    ...
    請修改Pokemon的範例程式
    運用運算子重載，將原先p1攻擊p2的呼叫方式 p1.Attack(p2); 改成 p1 >> p2;
    運用運算子重載，將原先pc治愈pokemon的 pc1.Cure(p1); 改寫成 pc1 << p1;
